...
data ExceptionCode
   = -- | The function code received in the query is not an allowable
     -- action for the server. This may be because the
     ...
     -- the server is in the wrong state to process a
     ...
     -- address for the server. More specifically, the
   ...
   | IllegalDataAddress
     ...
     -- value for server. This indicates a fault in the
   ...
   | ServerDeviceFailure
     ...
     -- server has accepted the request and is processing
   ...
   | Acknowledge
     -- | Specialized use in conjunction with programming commands. The
     -- server is engaged in processing a long–duration
