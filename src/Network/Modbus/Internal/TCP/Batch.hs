...
module Network.Modbus.Internal.TCP.Batch
  ( -- * Sessions
    Session
  , runSession
...
import "this" Network.Modbus.Internal.TCP.Protocol
...
prettyCommand = \case
    ...
    CmdCommand tpu fc bs ->
        "CmdCommand " <> unwords [prettyTpu tpu, show fc, show (BB.toLazyByteString bs)]
    CmdReadCoils tpu range ->
        "CmdReadCoils " <> unwords [prettyTpu tpu, prettyAddressRange range]
    CmdReadDiscreteInputs tpu range ->
        "CmdReadDiscreteInputs " <> unwords [prettyTpu tpu, prettyAddressRange range]
    CmdReadHoldingRegisters tpu range ->
        "CmdReadHoldingRegisters " <> unwords [prettyTpu tpu, prettyAddressRange range]
    CmdReadInputRegisters tpu range ->
        "CmdReadInputRegisters " <> unwords [prettyTpu tpu, prettyAddressRange range]
    CmdWriteSingleCoil tpu addr value ->
        "CmdWriteSingleCoil " <> unwords [prettyTpu tpu, prettyAddress addr, show value]
    CmdWriteMultipleCoils tpu addr values ->
        "CmdWriteMultipleCoils " <> unwords [prettyTpu tpu, prettyAddress addr, show values]
    CmdWriteSingleRegister tpu addr value ->
        "CmdWriteSingleRegister " <> unwords [prettyTpu tpu, prettyAddress addr, show value]
    CmdWriteMultipleRegisters tpu addr values ->
        "CmdWriteMultipleRegisters " <> unwords [prettyTpu tpu, prettyAddress addr, show values]
